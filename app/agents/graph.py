# -*- coding: utf-8 -*-
"""Deterministic state workflow for industrial alarm disposal.
...
from app.agents.db import insert_disposal_step, insert_work_order, update_work_order
from app.agents.linkage import build_emergency_linkage_plan
from app.agents.notifications import NotificationResult, send_wechat_notification_detail
...
def _log_step(state: DisposalState, step_name: str, content: str) -> None:
    ...
    try:
        ...
        insert_disposal_step(event_id, step_name, content[:1000], ts)

    except Exception as exc:
        logger.warning("Failed to persist disposal step: %s", exc)
...
def emergency_linkage(state: DisposalState) -> DisposalState:
    ...
    """Generate rule-based linkage instructions for a HIGH-level alarm."""
    ...
    result = build_emergency_linkage_plan(alarm_data)
...
def dispatch_order(state: DisposalState) -> DisposalState:
    ...
    notification_result = NotificationResult(False, "not_requested", "本次处置未请求发送企业微信通知。")
    if state.get("notification_enabled", True):
        notification_result = send_wechat_notification_detail(
    ...
    notification_sent = notification_result.ok
    state["notification_sent"] = notification_sent
    state["notification_status"] = notification_result.status
    state["notification_detail"] = notification_result.detail
...
def final_summary(state: DisposalState) -> DisposalState:
    ...
    if order_id and order_id != "N/A":
        ...
        try:
            ...
            if not update_work_order(order_id, report, status="completed"):
                logger.warning("Work order %s was not found during completion", order_id)
        except Exception as exc:
            logger.warning("Failed to complete work order %s: %s", order_id, exc)
...
def run_disposal(alarm_json: dict | str, *, send_notification: bool = True) -> dict:

    """Execute the complete rule-based disposal workflow.


    Args:

        alarm_json: Alarm JSON dict or JSON string.
        send_notification: Whether the notification adapter may send externally.
    ...
    state: DisposalState = {
        ...
        "notification_detail": "本次处置未请求发送企业微信通知。",
    ...
    try:
        ...
        state["steps_log"].append({
            "step": "system_error",
            "content": f"Pipeline execution failed: {e}",
...
def _build_result(state: DisposalState) -> dict:
    ...
    return {
        ...
        "notification_detail": state.get("notification_detail", "本次处置未请求发送企业微信通知。"),
